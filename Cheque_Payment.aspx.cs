...
{
    public partial class Cheque_Payment : System.Web.UI.Page
    {
        int cid;
        ...
        {
            string str = " select * from Cheque_Payment o inner join Cheque_Payment_Grv g on o.Cheque_Id = g.Cheque_Id where o.Cheque_Id =" + id.ToString();
        ...
        private void SetInitialRow()
        {
            ...
            dt.Columns.Add(new DataColumn("Cheque_Type", typeof(string)));
            ...
            dr["Cheque_Type"] = string.Empty;
            dr["Date"] = string.Empty;
            dr["Cheque_No"] = string.Empty;
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        DropDownList box2 = (DropDownList)Gridview1.Rows[rowIndex].Cells[1].FindControl("ddl_Cheque_Type");
                        TextBox box3 = (TextBox)Gridview1.Rows[rowIndex].Cells[2].FindControl("txt_Transfer_Date");
                        TextBox box4 = (TextBox)Gridview1.Rows[rowIndex].Cells[3].FindControl("txt_Cheque_No");
                        TextBox box5 = (TextBox)Gridview1.Rows[rowIndex].Cells[4].FindControl("txt_Bank_Name");
                        TextBox box6 = (TextBox)Gridview1.Rows[rowIndex].Cells[5].FindControl("txt_Amount");
                        ...
                        drCurrentRow["Cheque_Type"] = box2.SelectedItem.Text;
                        drCurrentRow["Date"] = box3.Text;
                        drCurrentRow["Cheque_No"] = box4.Text;
                        drCurrentRow["Bank_name"] = box5.Text;
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 1; i < dt.Rows.Count; i++)
                    {
                        ...
                        box2.Items.FindByText(dt.Rows[i]["Cheque_Type"].ToString()).Selected = true;
                        box3.Text = dt.Rows[i]["Date"].ToString();
                        box4.Text = dt.Rows[i]["Cheque_No"].ToString();
                        box5.Text = dt.Rows[i]["Bank_name"].ToString();
        ...
        private bool InsertRecords(StringCollection sc, int id)
        {
            ...
            foreach (string item in sc)
            {
                if (cid == 0)
                {
                    const string sqlStatement = "INSERT INTO Cheque_Payment_Grv (Cheque_Id, Payment_Type, Cheque_Type, Date, Cheque_No, Bank_name, Amount) VALUES";
                    if (item.Contains("|"))
                    {
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            if (cid == 0)
            {
                string strinsert = @"insert into dbo.Cheque_Payment(Customer_Code, Invoice_No ) 
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (rs > 0)
                    {
                        ...
                        if (r)
                        {
                            ...
                            Response.Redirect("Cheque_Payment.aspx");
            ...
            else
            {
                ...
                string strinsert = @"update Cheque_Payment set Customer_Code = '" + ddl_Cust_Code.SelectedItem.ToString().Trim() + "'where Cheque_Id = " + cid;
                bool res = new CommonUtil().InserUpdateQuery(strinsert);
                DataTable dt = new CommonUtil().GetData("select max(Cheque_Id) from Cheque_Payment");
                if (dt.Rows.Count > 0)
                {
